...
How your app uses the file service: **upload**, **public URLs**, **private access**, and **delete**.
...
*(If your CDN is configured with a different path, use that; the important part is using **cdn.nbericmmsu.com** for public reads and **objectKey** from the API. Only the `avatars/` folder is configured for public read access by default; other folders are private and should be accessed via signed URLs.)*

---

## Private access via signed URL

For **non-public folders** (anything outside `avatars/`), or when you want short-lived access, use a **signed GET URL**:
...
- `objectKey` (required) – storage key returned by the upload flow (e.g. `task-chat/13/1739123456789_file.png`).

**Example**

```http
GET https://files.nbericmmsu.com/api/files/signed-url?objectKey=task-chat/13/1739123456789_file.png
Authorization: Bearer <token>
```
...
- `signedUrl` – time-limited GET URL to read the object.
- `expiresIn` – seconds until the URL expires (default `900`, configurable via `SIGNED_URL_EXPIRY`).

Security rules:

- The bucket is derived from the authenticated user's `appId` in the token; **clients never specify the bucket**.
- The `objectKey` must match the pattern generated by this API and **belong to the authenticated user** (same `userId` segment), otherwise `403` is returned.
...
| Public file (avatars only) | GET | `https://cdn.nbericmmsu.com/...` | No (public) |
| Private file (signed) | GET | `https://files.nbericmmsu.com/api/files/signed-url?objectKey=...` | Bearer |
